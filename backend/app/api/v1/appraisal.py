...
@router.get(
    "/redMugImage",
    summary="Get Red Mug Image (Test Endpoint)",
    description="Returns a static image of a red mug for testing purposes.",
    responses={
        200: {
            "content": {"image/jpeg": {}},
            "description": "The image file in bytes.",
        },
        404: {
...
async def return_red_mug_image():
    """Returns a static test image (red mug)."""
    try:
        # Construct the path to the test image.
        image_path = os.path.expanduser('~/Projects/SnapValue/backend/images/red_mug.jpg')
        ...
        if not os.path.exists(image_path):
            raise HTTPException(
                ...
                detail={
                    "error_code": "NOT_FOUND",
                    "message": f"Test image not found at {image_path}",
                },
            )
        ...
        logger.error(f"Error getting red mug test image: {e}")
        raise HTTPException(
            status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
            detail={
                "error_code": "INTERNAL_ERROR",
                "message": "Failed to retrieve test image.",
            },
        )
