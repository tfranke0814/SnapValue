...
class TestAppraisalService:
    ...
    def test_validate_input_invalid_no_image(self, db_session):
        ...
        # In the new flow, this is still invalid because there's no image source
        assert service.validate_input(data) is False
    
    def test_validate_input_invalid_data_type(self, db_session):
        """Test input validation with invalid data type"""
        service = AppraisalService(db_session)
        assert service.validate_input("not_a_dict") is False

    @pytest.mark.asyncio
    async def test_submit_appraisal_from_upload_success(self, db_session, create_user):
        """Test successful appraisal submission from an upload"""
        service = AppraisalService(db_session)
        user = create_user(email="test@example.com")

        # Mock dependencies
        service.storage_service = AsyncMock()
        service.storage_service.upload_file.return_value = {
            'blob_name': 'images/test.jpg',
            'public_url': 'http://fake-storage.com/test.jpg'
        }
        
        service.ai_service = AsyncMock()
        service.ai_service.get_appraisal_from_agent.return_value = {
            'estimated_value': 500,
            'price_range': {'min': 480, 'max': 520},
            'confidence_score': 0.9
        }

        mock_upload_file = AsyncMock()
        mock_upload_file.read.return_value = b"fake_image_bytes"
        mock_upload_file.filename = "test.jpg"

        with patch('app.services.appraisal_service.asyncio.run') as mock_asyncio_run:
            # This mocks the async task submission in production
            mock_asyncio_run.return_value = "mock_task_id"

            result = await service.submit_appraisal_from_upload(
            ...
            # Verify that the underlying submission logic was called correctly
            # In dev mode, it processes synchronously, so we check the DB record
            appraisal_record = db_session.query(Appraisal).filter(Appraisal.id == result['appraisal_id']).first()
            assert appraisal_record is not None
            assert appraisal_record.user_id == user.id
            assert appraisal_record.status == 'completed' # Because it runs synchronously in test/dev
            assert appraisal_record.market_price == 500

    @pytest.mark.asyncio
    async def test_process_appraisal_async_flow(self, db_session, create_user):
        """Test the full async processing pipeline"""
        service = AppraisalService(db_session)
        user = create_user(email="test2@example.com")
        appraisal_id = str(uuid.uuid4())

        # Create a dummy appraisal record to be updated
        appraisal_record = Appraisal(id=appraisal_id, user_id=user.id, status="pending")
        db_session.add(appraisal_record)
        db_session.commit()

        # Mock dependencies
        service.storage_service = Mock()
        service.storage_service.upload_file.return_value = {
            'blob_name': 'images/test_async.jpg',
            'public_url': 'http://fake-storage.com/test_async.jpg'
        }
        ...
        # Run the async processing method
        final_result = await service._process_appraisal_async(
            appraisal_id=appraisal_id,
            file_content=b"fake_async_bytes",
            filename="test_async.jpg"
        )

        assert final_result['appraisal_id'] == appraisal_id
        assert final_result['estimated_value'] == 750
        assert final_result['confidence_score'] == 0.95

        # Check that the database record was finalized
        updated_record = db_session.query(Appraisal).filter(Appraisal.id == appraisal_id).first()
        assert updated_record.status == 'completed'
        assert updated_record.market_price == 750
        assert updated_record.price_range_max == 800
    
    def test_submit_appraisal_with_file(self, mock_ai, mock_vision, mock_storage, 
                                       db_session, create_user, test_image_file):
        """DEPRECATED: Test submitting appraisal with file upload"""
        pytest.skip("This test is deprecated as it reflects the old, non-agent-based architecture.")
